...
. $mt/checks/require.sh VCS_PAT GPG_KEY

. $mt/checks/file.sh "${GPG_KEY}"

gpg --batch --import "${GPG_KEY}"
. $mt/checks/success.sh $? 'GPG import key error!'
...
. $mt/checks/eq.sh 1 "${#ACTUAL_KEY_ID[@]}" 'Get GPG key ID error!'
ACTUAL_KEY_ID="${ACTUAL_KEY_ID[0]}"
ACTUAL_KEY_ID=(${ACTUAL_KEY_ID//:/ })
ACTUAL_KEY_ID="${ACTUAL_KEY_ID[4]}"
. $mt/checks/eq.sh "${GPG_KEY_ID}" "${ACTUAL_KEY_ID}" 'Wrong GPG key!'
...
. $mt/checks/success.sh $? 'Get user error!'
. $mt/checks/file.sh "${ISSUER}"
...
. $mt/checks/success.sh $? 'Config error!'
