...
ARTIFACT_FILE="lib/build/outputs/aar/${ARTIFACT_ID}-${VERSION}.aar"
. $mt/checks/file.sh "${ARTIFACT_FILE}"

SOURCES_FILE="lib/build/sources/${VARIANT}/${ARTIFACT_ID}-${VERSION}-sources.jar"
. $mt/checks/file.sh "${SOURCES_FILE}"

POM_FILE="lib/build/xml/${VARIANT}/${ARTIFACT_ID}-${VERSION}.pom"
