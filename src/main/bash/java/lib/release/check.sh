...
. $mt/checks/require.sh REPOSITORY_NAME REPOSITORY_VERSION ARTIFACT_ID ARTIFACT_VERSION

. $mt/checks/eq.sh "${REPOSITORY_NAME}" "${ARTIFACT_ID}" \
 "Repository name is \"${REPOSITORY_NAME}\", but artifact ID is \"${ARTIFACT_ID}\"!"

. $mt/checks/eq.sh "${REPOSITORY_VERSION}" "${ARTIFACT_VERSION}" \
