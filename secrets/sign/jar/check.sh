...
. $mt/checks/success $? "Get signature of \"${ISSUER}\" error!"

SIGNER="$(openssl "${KEYSTORE_TYPE}" -in "${KEYSTORE}" -nokeys -nodes -passin "pass:${KEYSTORE_PASSWORD}")"

. $mt/checks/success $? "Get signer of \"${KEYSTORE}\" error!"

KEY="$(openssl "${KEYSTORE_TYPE}" -in "${KEYSTORE}" -nocerts -nodes -passin "pass:${KEYSTORE_PASSWORD}")"

. $mt/checks/success $? "Get key of \"${KEYSTORE}\" error!"

. $mt/checks/require SIGNATURE SIGNER KEY
...
. $mt/checks/success $? "Check expected signature of \"${ISSUER}\" error!"

openssl cms -verify -noverify -content <(echo "${SIGNATURE}") -inform DER \
 -in <(unzip -p "${ISSUER}" "META-INF/${KEY_ALIAS^^}.RSA") &> /dev/null

. $mt/checks/success $? "Check actual signature of \"${ISSUER}\" error!"
